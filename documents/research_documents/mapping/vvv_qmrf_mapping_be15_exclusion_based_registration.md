...
# RCA: BE15 / N_BE_00015 — Exclusion-Based Registration (current filename: vvv_qmrf_mapping_be15_exclusion_based_registration.md; legacy filename: BE15_Apoha_exclusion.md)
...
| Layer | Label | Use |
|---|---|---|
| Current VVV-QMRF filename | vvv_qmrf_mapping_be15_exclusion_based_registration.md | Use as the normalized current filename |
...
|---|---|---|
| Solid arrows inside `Formal Semantic Schema` | Internal semantic construction: term, excluded other, exclusion-based meaning, and non-realist boundary. | A physical process or Quantum Mechanics state transition. |
| `ED_BE_00016` and other solid BE arrows | Internal Buddhist Epistemology relation or contextual dependency. | Cross-system identity with Quantum Measurement. |
| Dashed BE arrows such as `does not directly apprehend`, `explains general meaning without realism`, and `inference also differentiates by exclusion` | Boundary-sensitive BE explanation paths. | Direct perception of universals or a physical exclusion mechanism. |
| `structural analogy` | Bounded comparison between BE15 and QM structures such as PVM or complementarity. | Identity, proof, or replacement of Quantum Mechanics. |
| `PVM --> BOUND`, `COMP --> BOUND` | The QM analogue points must terminate in the RCA boundary. | Boundary-free equivalence between exclusion-based registration and QM formalism. |
